...
fig, (ax1) = plt.subplots(1) #create a matplotlib figure and a subplot (ax1) for displaying the image
...
for region in regionprops(label_image):#iterate over each connected region found in the labeled image
    ...
    rectBorder = patches.Rectangle((minCol, minRow), maxCol-minCol, maxRow-minRow, edgecolor="red", linewidth=2, fill=False)#creates a patch or a shape in matplotlib of a red rectangle around the detected region in the image
    ax1.add_patch(rectBorder)#adds the rectangle over the detected region
    #lets draw a red rectangle over those regions
